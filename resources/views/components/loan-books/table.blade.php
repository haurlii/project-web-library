    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                <td class="px-8 py-3">
                    <div class="flex items-center font-medium whitespace-nowrap mr-3 max-w-xl">
                        <img src="{{ $loan->user->avatar ? asset('storage/' . $loan->user->avatar) :
                            ...
                            alt="{{ $loan->user->firstname . $loan->user->lastname }}"
                ...
                <td class="px-8 py-3">
                    <div class="flex items-center font-medium whitespace-nowrap mr-3 max-w-xl">
                        <img src="{{ $loan->book->cover ? asset('storage/' . $loan->book->cover) :
                                    asset('assets/images/user/user-default.png') }}" alt="{{ $loan->book->title }}"
                            class="h-8 w-auto mr-3 rounded-full">
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-6">
                            ...
                            <div class="col-span-1 sm:col-span-3">
                                ...
                                <input type="text" name="name" id="user_id"
                                    value="{{ $loan->user->firstname . ' ' . $loan->user->lastname }}" readonly
                                    class="h-11 w-full rounded-lg border px-4 py-2.5 text-sm shadow-theme-xs @error('user_id') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror border-gray-300 text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                <input type="hidden" name="user_id" id="user_id" value="{{ $loan->user->id }}"
                                    placeholder="Enter the user" autocomplete="off" />
                                @error('user_id')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
            ...
            {{--
            <!-- Delete Modal -->
            <div id="deleteLoanBookModal-{{ $loan->id }}"
                class="fixed inset-0 hidden items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="deleteLoanBookModal-{{ $loan->id }}"
                    data-modal-toggle="deleteLoanBookModal-{{ $loan->id }}"
                    class="modal-close-btn fixed inset-0 h-full w-full bg-gray-300/50 backdrop-blur-[3rem]"></div>
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="deleteLoanBookModal-{{ $loan->id }}"
                        data-modal-toggle="deleteLoanBookModal-{{ $loan->id }}"
                        class="absolute right-3 top-3 z-999 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white sm:right-6 sm:top-6 sm:h-11 sm:w-11">
                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="text-center">
                        ...
                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteLoanBookModal-{{ $loan->id }}"
                                data-modal-toggle="deleteLoanBookModal-{{ $loan->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="/books/{{ $loan->slug }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
