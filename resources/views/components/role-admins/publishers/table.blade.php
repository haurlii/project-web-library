    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-4 py-3">Nama Penerbit</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                <th scope="row" class="px-4 py-3 font-medium text-gray-900 max-w-lg whitespace-nowrap dark:text-white">
                    {{ $loop->iteration }}</th>
                <td class="px-4 py-3 max-w-[12rem] truncate">
                    <div class="flex items-center mr-3">
                        <img src="{{ $publisher->logo ? asset('storage/' . $publisher->logo) :
                                    asset('assets/images/user/user-default.png') }}" alt="{{ $publisher->name }}"
                            class="h-8 w-8 mr-3 rounded-full">
                        {{ $publisher->name }}
                    </div>
                </td>
                <td class="px-4 py-3 max-w-2xl truncate">{{ $publisher->address ?? 'Tidak Tercantum' }}</td>
                <td class="px-4 py-3">{{ $publisher->email ?? 'Tidak Tercantum' }}</td>
                <td class="px-4 py-3">{{ $publisher->contact ?? 'Tidak Tercantum' }}</td>
                <td class="px-4 py-3 flex items-center justify-end">
                    <button id="publisher-{{ $publisher->id }}-dropdown-button"
                        data-dropdown-toggle="publisher-{{ $publisher->id }}-dropdown"
                    ...
                    <div id="publisher-{{ $publisher->id }}-dropdown"
                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-sm" aria-labelledby="publisher-{{ $publisher->id }}-dropdown-button">
                            <li>
                                <button type="button" data-modal-target="updatePublisherModal-{{ $publisher->id }}"
                                    data-modal-toggle="updatePublisherModal-{{ $publisher->id }}"
                            ...
                            <li>
                                <button type="button" data-modal-target="deletePublisherModal-{{ $publisher->id }}"
                                    data-modal-toggle="deletePublisherModal-{{ $publisher->id }}"
            ...
            <!-- Update Modal -->
            <div id="updatePublisherModal-{{ $publisher->id }}"
                class="hidden fixed inset-0 items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="updatePublisherModal-{{ $publisher->id }}"
                    data-modal-toggle="updatePublisherModal-{{ $publisher->id }}"
                ...
                <div class="relative w-full max-w-[960px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="updatePublisherModal-{{ $publisher->id }}"
                        data-modal-toggle="updatePublisherModal-{{ $publisher->id }}"
                    ...
                    <form action="{{ route('admin.publishers.update', $publisher->slug) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-6">
                            <div class="col-span-1 sm:col-span-3">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') ?? $publisher->name }}"
                                    autofocus autocomplete="off" placeholder="Masukkan nama penerbit"
                            ...
                            <div class="col-span-1 sm:col-span-3">
                                ...
                                <input type="text" name="address" id="address"
                                    ...
                                    placeholder="Masukkan alamat penerbit" autocomplete="off"
                            ...
                            <div class="col-span-1 sm:col-span-3">
                                ...
                                <input type="text" name="contact" id="contact"
                                    value="{{ old('contact') ?? $publisher->contact }}"
                                    placeholder="Masukan kontak penerbit" autocomplete="off"
                            ...
                            <div class="col-span-1 sm:col-span-3">
                                ...
                                <input type="email" name="email" id="email"
                                    value="{{ old('email') ?? $publisher->email }}" placeholder="Masukan email penerbit"
                                    autocomplete="off"
                            ...
                            <div class="col-span-1 sm:col-span-6">
                                <label for="logo-update-{{ $publisher->id }}"
                                    class=" mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Upload File
                                </label>
                                <input
                                    class="@error('logo') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer placeholder:text-gray-900 dark:text-white/90 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:placeholder-white/90 focus:file:ring-brand-300 transition-colors file:mr-5 file:border-collapse file:cursor-pointer file:rounded-l-lg file:border-0 file:border-r file:border-solid file:border-gray-200 file:bg-gray-50 file:text-xs file:text-gray-400  dark:file:border-gray-700 dark:file:bg-gray-900 dark:file:text-white/30"
                                    aria-describedby="user_avatar_help" id="logo-update-{{ $publisher->id }}"
                                    name="logo" type="file" accept="image/*" />
                                <input name="logo" id="logo-hidden-{{ $publisher->id }}" type="hidden"
                                    accept="image/*" />
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                <div class="flex items-center justify-start m-3">
                                    <img src="{{ $publisher->logo ? asset('storage/' . $publisher->logo) : asset('assets/images/user/user-default.png') }}"
                                        alt="{{ $publisher->name }}" class="h-[96px] w-auto rounded-sm" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end w-full gap-3 mt-6">
                            <button data-modal-target="updatePublisherModal-{{ $publisher->id }}"
                                data-modal-toggle="updatePublisherModal-{{ $publisher->id }}" type="button"
            ...
            <div id="deletePublisherModal-{{ $publisher->id }}"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus data penerbit buku yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deletePublisherModal-{{ $publisher->id }}"
                                data-modal-toggle="deletePublisherModal-{{ $publisher->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="{{ route('admin.publishers.destroy', $publisher->slug) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
