    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-4 py-3">Nama Penulis</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3">
                        <img src="{{ $author->avatar ? asset('storage/' . $author->avatar) :
                                    asset('assets/images/user/user-default.png') }}" alt="{{ $author->name }}"
                            class="h-8 w-8 mr-3 rounded-full">
                        {{ $author->name }}
                    </div>
                </td>
                <td class="px-4 py-3 flex items-center justify-end">
                    <button id="author-{{ $author->id }}-dropdown-button"
                        data-dropdown-toggle="author-{{ $author->id }}-dropdown"
                    ...
                    <div id="author-{{ $author->id }}-dropdown"
                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-sm" aria-labelledby="author-{{ $author->id }}-dropdown-button">
                            <li>
                                <button type="button"
                                    class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200"
                                    data-modal-target="updateAuthorModal-{{ $author->id }}"
                                    data-modal-toggle="updateAuthorModal-{{ $author->id }}">
                            ...
                            <li>
                                <button type="button" data-modal-target="deleteAuthorModal-{{ $author->id }}"
                                    data-modal-toggle="deleteAuthorModal-{{ $author->id }}"
            ...
            <div id="updateAuthorModal-{{ $author->id }}"
                ...
                <div class="relative w-full max-w-[584px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="updateAuthorModal-{{ $author->id }}"
                        data-modal-toggle="updateAuthorModal-{{ $author->id }}"
                    ...
                    <form action="{{ route('admin.authors.update', $author->slug) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-4">
                            ...
                            <div class="col-span-1 sm:col-span-4">
                                <label for="avatar-update-{{ $author->id }}"
                                    class=" mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Upload File
                                </label>
                                <input
                                    class="@error('avatar') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror avatar-update block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 placeholder:text-gray-900 dark:text-white/90 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:placeholder-white/90 focus:file:ring-brand-300 transition-colors file:mr-5 file:border-collapse file:cursor-pointer file:rounded-l-lg file:border-0 file:border-r file:border-solid file:border-gray-200 file:bg-gray-50 file:text-xs file:text-gray-400  dark:file:border-gray-700 dark:file:bg-gray-900 dark:file:text-white/30"
                                    aria-describedby="user_avatar_help" id="avatar-update-{{ $author->id }}"
                                    name="avatar" type="file" />
                                <input name="avatar" id="avatar-hidden-{{ $author->id }}" type="hidden"
                                    accept="image/*">
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                <div class="flex items-center justify-start m-3">
                                    <img src="{{ $author->avatar ? asset('storage/' . $author->avatar) : asset('assets/images/user/user-default.png') }}"
                                        alt="{{ $author->name }}" class="h-[96px] w-auto rounded-sm">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end w-full gap-3 mt-6">
                            <button data-modal-target="updateAuthorModal-{{ $author->id }}"
                                data-modal-toggle="updateAuthorModal-{{ $author->id }}" type="button"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    ...
                    <div class="text-center">
                        ...
                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteAuthorModal-{{ $author->id }}"
                                data-modal-toggle="deleteAuthorModal-{{ $author->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
