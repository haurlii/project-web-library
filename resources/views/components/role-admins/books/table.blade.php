    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3 whitespace-nowrap w-16">Kode Buku</th>
                <th scope="col" class="px-6 py-3 whitespace-nowrap w-16">Judul Buku</th>
                <th scope="col" class="px-6 py-3 sr-only">Judul Buku</th>
                <th scope="col" class="px-6 py-3">ISBN</th>
                <th scope="col" class="px-6 py-3">Bahasa</th>
                <th scope="col" class="px-6 py-3 whitespace-nowrap w-16">Tahub Terbit</th>
                <th scope="col" class="px-6 py-3 whitespace-nowrap w-16">Jumlah Halaman</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                <th scope="row"
                    class="px-6 py-3 font-medium text-gray-900 max-w-lg truncate whitespace-nowrap dark:text-white">
                    {{ $loop->iteration }}</th>
                <td class="px-6 py-3">
                ...
                <td class="pl-6 py-3">
                    <img src="{{ $book->cover ? asset('storage/' . $book->cover) : asset('assets/images/cover-default.png') }}"
                        alt="{{ $book->title }}" class="h-24 w-24 mr-3 rounded-sm object-contain">
                </td>
                <td class="pr-6 py-3">
                    <div class="flex items-center font-medium mx-3 whitespace-nowrap max-w-2xl">
                        {{ Str::of($book->title)->limit(50) }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                ...
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        {{ $book->publication_year ?? 'Tidak tersedia' }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        {{ $book->number_of_pages ?? 'Tidak tersedia' }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-xl">
                        {{ Str::of($book->author->name)->limit(100) ?? 'Tidak tersedia' }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-2xl">
                        {{ Str::of($book->publisher->name)->limit(100) ?? 'Tidak tersedia' }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-2xl">
                        {{ Str::of($book->category->name)->limit(50) ?? 'Tidak tersedia' }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                ...
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-2xl">
                        {{ Str::of($book->synopsis ?? 'Tidak tersedia')->limit(100) }}
                    </div>
                </td>
                <td class="px-6 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                ...
                <td class="px-6 py-3 flex items-center justify-end">
                    <button id="book-{{ $book->id }}-dropdown-button"
                        data-dropdown-toggle="book-{{ $book->id }}-dropdown"
                    ...
                    <div id="book-{{ $book->id }}-dropdown"
                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-sm" aria-labelledby="book-{{ $book->id }}-dropdown-button">
                            <li>
                                <button type="button"
                                    class="flex w-full items-center py-2 px-6 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200"
                                    data-modal-target="updateBookModal-{{ $book->id }}"
                                    data-modal-toggle="updateBookModal-{{ $book->id }}">
                            ...
                            <li>
                                <button type="button" data-modal-target="deleteBookModal-{{ $book->id }}"
                                    data-modal-toggle="deleteBookModal-{{ $book->id }}"
                                    class="flex w-full items-center py-2 px-6 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                ...
                {{-- <td class="px-6 py-3 flex items-center space-x-4">
                    <button type="button" data-modal-target="loanBookModal-{{ $book->id }}"
                        data-modal-toggle="loanBookModal-{{ $book->id }}"
            ...
            <!-- Update Modal -->
            <div id="updateBookModal-{{ $book->id }}"
                class="hidden fixed inset-0 items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="updateBookModal-{{ $book->id }}"
                    data-modal-toggle="updateBookModal-{{ $book->id }}"
                    class="modal-close-btn fixed inset-0 h-full w-full bg-gray-300/50 backdrop-blur-[3rem]"></div>
                <div class="relative w-full max-w-[1200px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="updateBookModal-{{ $book->id }}"
                        data-modal-toggle="updateBookModal-{{ $book->id }}"
                    ...
                    <form action="{{ route('admin.books.update', $book->slug) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-8">
                            ...
                            <div class="col-span-1 sm:col-span-4">
                                ...
                                <input type="text" name="isbn" id="isbn" value="{{ old('isbn') ?? $book->isbn }}"
                                    placeholder="e.g., 978-XXXXXXXXXX" autocomplete="off"
                                    class="@error('isbn') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-6 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                @error('isbn')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select name="language" id="language"
                                        class="@error('language') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-none px-6 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                        ...
                                        <option value="{{ $language->value }}"
                                            ...
                                            (old('language') ?? $book->language->value) == $language->value )>
                                            {{ $language->value }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select name="publication_year" id="publication_year"
                                        class="@error('publication_year') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-none px-6 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                        :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'"
                                        @change="isOptionSelected = true">
                                        <option value="" selected=""
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                            Silahkan Pilih Tahun Terbit
                                        </option>
                                        @for ( $year = now()->year; $year >= 1900; $year-- )
                                        <option value="{{ $year }}"
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @selected(
                                            (old('publication_year') ?? $book->publication_year) == $year )>
                                            {{ $year }}
                                        </option>
                                        @endfor
                                    </select>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="text" name="number_of_pages" id="number_of_pages"
                                    value="{{ old('number_of_pages') ?? $book->number_of_pages }}"
                                    placeholder="e.g., 0xx" autocomplete="off"
                                    class="@error('number_of_pages') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-6 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="text" name="price" id="price" value="{{ old('price') ?? $book->price }}"
                                    placeholder="e.g., Rp 1xxxxx" autocomplete="off"
                                    class="@error('price') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-6 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                @error('price')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="text" name="total" id="total"
                                    value="{{ old('total') ?? $book->stock?->total }}" placeholder="e.g., Rp 1xxxxx"
                                    autocomplete="off" readonly
                                    class="@error('total') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-6 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                                @error('total')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select name="author_id" id="author_id" value="{{ old('author_id') }}"
                                        class="@error('author_id') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-none px-6 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                        :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'"
                                        @change="isOptionSelected = true">
                                        <option value="" selected=""
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        ...
                                        <option value="{{ $author->id }}"
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @selected(
                                            (old('author_id') ?? $book->author->id) == $author->id )>
                                            {{ $author->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select name="publisher_id" id="publisher_id" value="{{ old('publisher_id') }}"
                                        class="@error('publisher_id') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-none px-6 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                        :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'"
                                        @change="isOptionSelected = true">
                                        <option value="" selected=""
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        ...
                                        <option value="{{ $publisher->id }}"
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @selected(
                                            (old('publisher_id') ?? $book->publisher->id) == $publisher->id )>
                                            {{ $publisher->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select name="category_id" id="category_id" value="{{ old('category_id') }}"
                                        class="@error('category_id') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-none px-6 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                        :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'"
                                        @change="isOptionSelected = true">
                                        <option value="" selected=""
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        ...
                                        <option value="{{ $category->id }}"
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @selected(
                                            (old('category_id') ?? $book->category->id) == $category->id )>
                                            {{ $category->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-1 sm:col-span-8">
                                ...
                                <textarea placeholder="Enter a synopsis..." type="text" rows="6" name="synopsis"
                                    id="synopsis" autocomplete="off"
                                    class="@error('synopsis') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror  dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 px-6 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('synopsis') ?? $book->synopsis }}</textarea>
                                @error('synopsis')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-1">
                                <div class="flex items-center justify-center m-3">
                                    ...
                                    asset('assets/images/cover-default.png') }}" alt="{{ $book->cover }}"
                                        class="h-[96px] w-auto rounded-sm" id="avatar-preview-update">
                                </div>
                            </div>

                            <div class="col-span-1 sm:col-span-7">
                                <label for="cover-update-{{ $book->id }}"
                                    class=" mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Upload File
                                </label>
                                <input
                                    ...
                                    aria-describedby="user_avatar_help" data-preview-target="avatar-preview-create"
                                    id="cover-update-{{ $book->id }}" name="cover" type="file" accept="image/*" />
                                <input name="cover" id="cover-hidden-{{ $book->id }}" type="hidden" accept="image/*">
                        ...
                        <div class="flex items-center justify-end w-full gap-3 mt-6">
                            <button data-modal-target="updateBookModal-{{ $book->id }}"
                                data-modal-toggle="updateBookModal-{{ $book->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Close
                            </button>
                            <button type="submit"
                                class="flex justify-center w-full px-6 py-3 text-sm font-medium text-white rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600 sm:w-auto">
                                Save Changes
                            </button>
                        </div>
                    </form>
            ...
            <div id="deleteBookModal-{{ $book->id }}"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus judul buku yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteBookModal-{{ $book->id }}"
                                data-modal-toggle="deleteBookModal-{{ $book->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="{{ route('admin.books.destroy', $book->slug) }}" method="POST">
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="flex justify-center w-full px-6 py-3 text-sm font-medium text-white rounded-lg bg-warning-500 shadow-theme-xs hover:bg-warning-600 sm:w-auto">
                                    Delete
                                </button>
                            </form>
                        </div>
            ...
            <!-- Create modal -->
            {{-- <div id="loanBookModal"
                class="hidden fixed inset-0 items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="loanBookModal" data-modal-toggle="loanBookModal"
                    class="modal-close-btn fixed inset-0 h-full w-full bg-gray-300/50 backdrop-blur-[3rem]"></div>
                <div class="relative w-full max-w-[584px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="loanBookModal" data-modal-toggle="loanBookModal"
                        class="group absolute right-3 top-3 z-999 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-200 text-gray-500 transition-colors hover:bg-gray-300 hover:text-gray-500 dark:bg-gray-800 dark:hover:bg-gray-700 sm:right-6 sm:top-6 sm:h-11 sm:w-11">
                        <svg class="transition-colors fill-current group-hover:text-gray-600 dark:group-hover:text-gray-200"
                            width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <form action="/loan-books" method="POST" enctype="multipart/form-data">
                        @csrf
                        <h4 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">
                            Add Data Loan Book
                        </h4>
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-4">
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="name" name="name" id="user_id"
                                    value="{{ old('name') ?? $user->firstname . ' ' . $user->lastname }}"
                                    autocomplete="off" readonly
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div class="relative max-w-sm">
                                    ...
                                    <input id="loan_date" datepicker datepicker-format="dd MM yyyy" type="text"
                                        name="loan_date"
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div class="relative max-w-sm">
                                    ...
                                    <input id="due_date" datepicker datepicker-format="dd MM yyyy" type="text"
                                        name="due_date"
                        ...
                        <div class="flex items-center justify-end w-full gap-3 mt-6">
                            <button data-modal-target="loanBookModal" data-modal-toggle="loanBookModal" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Close
                            </button>
                            <button type="submit"
