...
<div class="overflow-x-auto">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                <td class="px-4 py-3 max-w-lg truncate">
                    <div class="flex items-center mr-3">
                        <img src="{{ $category->cover ? asset('storage/' . $category->cover) :
                                    asset('assets/images/user/user-default.png') }}" alt="{{ $category->name }}"
                            class="h-8 w-8 mr-3 rounded-full">
                        {{ $category->name }}
                    </div>
                </td>
                <td class="px-4 py-3 max-w-2xl truncate">{{ $category->description ?? 'Tidak tersedia' }}</td>
                <td class="px-4 py-3 flex items-center justify-end">
                    ...
                    <div id="category-{{ $category->id }}-dropdown"
                        ...
                        <ul class="py-1 text-sm" aria-labelledby="category-{{ $category->id }}-dropdown-button">
                            <li>
                                <button type="button" data-modal-target="updateCategoryModal-{{ $category->id }}"
                                    data-modal-toggle="updateCategoryModal-{{ $category->id }}"
                            ...
                            <li>
                                <button type="button" data-modal-target="deleteCategoryModal-{{ $category->id }}"
            ...
            <!-- Update Modal -->
            <div id="updateCategoryModal-{{ $category->id }}"
                class="hidden fixed inset-0 items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="updateCategoryModal-{{ $category->id }}"
                    data-modal-toggle="updateCategoryModal-{{ $category->id }}"
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-6">
                            <div class="col-span-1 sm:col-span-6">
                                ...
                                <input type="text" name="name" id="name" placeholder="Masukkan nama kategori..."
                                    value="{{ old('name') ?? $category->name }}" autofocus autocomplete="off"
                            ...
                            <div class="col-span-1 sm:col-span-6">
                                ...
                                <textarea placeholder="Masukkan deskripsi kategori..." type="text" rows="6"
                                    name="description" id="description" autocomplete="off"
                                    class="@error('description') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description') ?? $category->description }}</textarea>
                                @error('description')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-1">
                                <div class="flex items-center justify-center m-3">
                                    <img src="{{ $category->cover ? asset('storage/' . $category->cover) : asset('assets/images/user/user-default.png') }}"
                                        alt="{{ $category->name }}" class="h-[96px] w-auto rounded-sm">
                                </div>
                            </div>

                            <div class="col-span-1 sm:col-span-5">
                                <label for="cover-update-{{ $category->id }}"
                                    class=" mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Upload File
                                </label>
                                <input
                                    class="@error('cover') bg-red-50 dark:bg-red-900/20 border-red-500 text-red-600 placeholder-red-50 focus:ring-red-500/10 focus:border-red-300 dark:text-red-500 dark:placeholder-red-500 dark:border-red-800 dark:focus:ring-red-50/10 dark:focus:border-red-800 @enderror block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 placeholder:text-gray-900 dark:text-white/90 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:placeholder-white/90 focus:file:ring-brand-300 transition-colors file:mr-5 file:border-collapse file:cursor-pointer file:rounded-l-lg file:border-0 file:border-r file:border-solid file:border-gray-200 file:bg-gray-50 file:text-xs file:text-gray-400  dark:file:border-gray-700 dark:file:bg-gray-900 dark:file:text-white/30"
                                    aria-describedby="user_avatar_help" id="cover-update-{{ $category->id }}"
                                    name="cover" type="file" accept="image/*" />
                                <input name="cover" id="cover-hidden-{{ $category->id }}" type="hidden"
                                    accept="image/*">
            ...
            <div id="deleteCategoryModal-{{ $category->id }}"
                ...
                <div data-modal-target="deleteCategoryModal-{{ $category->id }}"
                    data-modal-toggle="deleteCategoryModal-{{ $category->id }}"
                    class="modal-close-btn fixed inset-0 h-full w-full bg-gray-300/50 backdrop-blur-[3rem]"></div>
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="deleteCategoryModal-{{ $category->id }}"
                        data-modal-toggle="deleteCategoryModal-{{ $category->id }}"
                        class="absolute right-3 top-3 z-999 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white sm:right-6 sm:top-6 sm:h-11 sm:w-11">
                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus data kategori buku yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteCategoryModal-{{ $category->id }}"
                                data-modal-toggle="deleteCategoryModal-{{ $category->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
