    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-8 py-3">Tersedia</th>
                <th scope="col" class="px-8 py-3">Dipinjam</th>
                <th scope="col" class="px-8 py-3">Rusak</th>
                <th scope="col" class="px-8 py-3">Hilang</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                <th scope="row"
                    ...
                    {{ $loop->iteration }}</th>
                <td class="pl-8 py-3">
                    <img src="{{ $stock->book->cover ? asset('storage/' . $stock->book->cover) : asset('assets/images/user/user-default.png') }}"
                        alt="{{ $stock->book->title }}" class="h-24 w-24 mr-3 rounded-sm">
                </td>
                <td class="pr-8 py-3">
                    <div class="flex items-center font-medium whitespace-nowrap mx-3">
                        {{ $stock->book->title }}
                    </div>
                </td>
                <td class="px-8 py-3">
                ...
                <td class="px-8 py-3 flex items-center justify-end">
                    <button type="button" data-modal-target="updateStockBookModal-{{ $stock->id }}"
                        data-modal-toggle="updateStockBookModal-{{ $stock->id }}"
            ...
            <div id="updateStockBookModal-{{ $stock->id }}"
                ...
                <div class="relative w-full max-w-[584px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="updateStockBookModal-{{ $stock->id }}"
                        data-modal-toggle="updateStockBookModal-{{ $stock->id }}"
                    ...
                    <form action="{{ route('admin.stock-books.update', $stock->id) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <h4 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">
                            Mengubah Data Stok Buku
                        </h4>
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="number" name="total" id="total" value="{{ old('total') ?? $stock->total }}"
                                    autofocus autocomplete="off" min="{{ $stock->total }}"
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="number" name="available" id="available"
                                    value="{{ old('available') ?? $stock->available }}" autofocus autocomplete="off"
                                    min="0" readonly
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="number" name="loan" id="loan" value="{{ old('loan') ?? $stock->loan }}"
                                    autofocus autocomplete="off" min="0" readonly
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-900 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/90 dark:focus:border-brand-800" />
                                @error('loan')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="numbber" name="damage" id="damage"
                                    value="{{ old('damage') ?? $stock->damage }}" autofocus autocomplete="off" min="0"
                                    readonly
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-900 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/90 dark:focus:border-brand-800" />
                                @error('damage')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="numbber" name="lost" id="lost" value="{{ old('lost') ?? $stock->lost }}"
                                    autofocus autocomplete="off" min="0" readonly
                                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-900 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/90 dark:focus:border-brand-800" />
                                @error('lost')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                        ...
                        <div class="flex items-center justify-end w-full gap-3 mt-6">
                            <button data-modal-target="updateStockBookModal-{{ $stock->id }}"
                                data-modal-toggle="updateStockBookModal-{{ $stock->id }}" type="button"
            ...
            <!-- Delete Modal -->
            <div id="deleteStockBookModal-{{ $stock->id }}"
                class="fixed inset-0 hidden items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="deleteStockBookModal-{{ $stock->id }}"
                    data-modal-toggle="deleteStockBookModal-{{ $stock->id }}"
                    class="modal-close-btn fixed inset-0 h-full w-full bg-gray-300/50 backdrop-blur-[3rem]"></div>
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="deleteStockBookModal-{{ $stock->id }}"
                        data-modal-toggle="deleteStockBookModal-{{ $stock->id }}"
                        class="absolute right-3 top-3 z-999 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white sm:right-6 sm:top-6 sm:h-11 sm:w-11">
                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="text-center">
                        ...
                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteStockBookModal-{{ $stock->id }}"
                                data-modal-toggle="deleteStockBookModal-{{ $stock->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-8 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="/stock-books/{{ $stock->slug }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
