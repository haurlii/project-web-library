...
<div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
    <form action="{{ route('admin.fine-settings.update', $fineSetting->id) }}" method="POST">
        @csrf
        @method('PATCH')
        <div class="-mx-2.5 flex flex-wrap gap-y-5">
            <div class="w-full px-2.5">
                <label for="late_fee_per_day" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Denda Keterlambatan <span class="text-sm font-extrabold">( per hari )</span>
                </label>
                <input type="number" id="late_fee_per_day" name="late_fee_per_day" {{--
                    value="{{ is_numeric($fineSetting->late_fee_per_day) ? $fineSetting->late_fee_per_day : '' }}" --}}
                    value="{{ old('late_fee_per_day') ?? $fineSetting->late_fee_per_day }}"
                    placeholder="Denda keterlambatan per hari"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
            </div>

            <div class="w-full px-2.5">
                <label for="damage_fee_percentage"
                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Denda Kerusakan <span class="text-sm font-extrabold">( % )</span>
                </label>
                <input type="number" id="damage_fee_percentage" name="damage_fee_percentage"
                    value="{{ old('damage_fee_percentage') ?? $fineSetting->damage_fee_percentage }}" {{--
                    value="{{ is_numeric($fineSetting->damage_fee_percentage) ? $fineSetting->damage_fee_percentage : '' }}"
                    --}} placeholder="Denda kerusakan %" min="0" max="100"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
            </div>

            <div class="w-full px-2.5">
                <label for="lost_fee_percentage"
                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Denda Kehilangan <span class="text-sm font-extrabold">( % )</span>
                </label>
                <input type="number" id="lost_fee_percentage" name="lost_fee_percentage"
                    value="{{ old('lost_fee_percentege') ?? $fineSetting->lost_fee_percentage }}" {{--
                    value="{{ is_numeric($fineSetting->lost_fee_percentage) ? $fineSetting->lost_fee_percentage : '' }}"
                    --}} placeholder="Denda kehilangan %" min="0" max="100"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
            </div>
